...
ALERT_COLUMNS = ["entity_id", "risk_score", "reason", "evidence", "top_signal",
                 "rule_score", "anomaly_score", "gnn_score", "correlation_score",
                 "taint_score", "contributions", "wallets", "suspicious_merge"]
...
def collect_signals(df: pd.DataFrame, cfg: dict) -> dict:
    ...
    correlation = (links.groupby("entity_id")["final_score"].max().rename("correlation_score")
                   if len(links) else pd.Series(dtype=float, name="correlation_score"))

    entity_graph = build_entity_graph(graph, features.clustering, cfg)
    taint = compute_taint(entity_graph, alerts, cfg=cfg)
    ...
    signals = signals.merge(correlation, on="entity_id", how="left")
...
def build_alerts(bundle: dict, stacker: Stacker, cfg: dict) -> pd.DataFrame:
    ...
    correlation_reason, correlation_ip = {}, {}
    if len(bundle["links"]):
        best = bundle["links"].sort_values("final_score", ascending=False)
        best = best.drop_duplicates("entity_id")
        correlation_reason = dict(zip(best["entity_id"], best["reason"]))
        correlation_ip = dict(zip(best["entity_id"], best["ip"]))
    ...
    for _, row in flagged.iterrows():
        ...
        if entity in correlation_ip:
            evidence.append(str(correlation_ip[entity]))
        explanation = explain_entity(row, stacker, baseline,
                                     reasons_by_entity.get(entity, []), evidence,
                                     correlation_reason.get(entity),
                                     list(row.get("taint_path") or []), cfg)
        rows.append({
...
def main(argv=None) -> None:
    ...
    print(json.dumps(run(args.input, args.ground_truth, args.parquet, args.js), indent=2))
